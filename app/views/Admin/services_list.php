                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>KSh <?= number_format($service['price'] ?? 0, 2) ?></td>
                                        <td>
                                            <span class="badge bg-<?= ($service['status'] ?? 'active') === 'active' ? 'success' : 'secondary' ?>">
                                                <?= ucfirst($service['status'] ?? 'active') ?>
                                            </span>
                                        </td>
                                        <td><?= date('M j, Y', strtotime($service['created_at'])) ?></td>
                                        <td>
                                            <a href="?action=edit_service&id=<?= $service['id'] ?>" class="btn btn-sm btn-warning">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <a href="?action=delete_service&id=<?= $service['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this service?')">
